...
日常 add 不要求预先 server add 或提供规则名：`--server` 始终必填，可直接引用 SSH 别名，不存在的服务器与转发在同一次配置事务中创建。即使只有一个已保存 profile 也不推断服务器。规则名可用 `--name` 指定，旧位置名称兼容；省略时生成 `服务器-方向-源端口`。方向参数不能吞掉位置名称；完整映射使用等号明确关联，兼容层只识别确为映射/端口的旧空格写法。
