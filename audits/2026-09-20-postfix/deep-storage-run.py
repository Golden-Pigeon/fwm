...
def main():
    with tempfile.TemporaryDirectory(prefix="fwm-deep-storage-", dir="/private/tmp") as tmp:
        directory = Path(tmp)
        shutil.copytree(SOURCE / "store", directory / "store")
        source = (SOURCE / "store.rs").read_text()
        (directory / "store.rs").write_text(source + '\n#[cfg(test)]\n#[path = "deep-storage.rs"]\nmod deep_review;\n')
        shutil.copy2(Path(__file__).with_name("deep-storage.rs"), directory / "deep-storage.rs")
    ...
    Path(__file__).with_name("deep-storage-results.json").write_text(json.dumps(output, ensure_ascii=False, indent=2) + "\n")
    print(result.stdout)
